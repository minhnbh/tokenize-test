...
export function removeChart(chartId: string) {
  // @ts-ignore
  let charts = JSON.parse(localStorage.getItem(CHARTS_KEY)) || [];
  charts = charts.filter((chart: { id: any }) => chart.id !== chartId);
  localStorage.setItem(CHARTS_KEY, JSON.stringify(charts));
  localStorage.removeItem(CHARTS_KEY + "." + chartId);
  return new Promise<void>((resolve) => resolve());
}

export function saveChart(chartData: { [x: string]: any; content: any }) {
  ...
  // @ts-ignore
  let charts = JSON.parse(localStorage.getItem(CHARTS_KEY)) || [];
  charts = charts.filter((chart: { id: any }) => chart.id !== info.id);
  charts.push(info);
  localStorage.setItem(CHARTS_KEY, JSON.stringify(charts));
...
export function getChartContent(chartId: string) {
...
export function removeStudyTemplate({ name }: any): Promise<unknown> {
  // @ts-ignore
  let studies = JSON.parse(localStorage.getItem(STUDIES_KEY)) || [];
  studies = studies.filter((study: { name: any }) => study.name !== name);
...
export function saveStudyTemplate({ content, ...info }: any) {
  // @ts-ignore
  let studies = JSON.parse(localStorage.getItem(STUDIES_KEY)) || [];
  studies = studies.filter((study: { name: any }) => study.name !== info.name);
  studies.push(info);
  localStorage.setItem(STUDIES_KEY, JSON.stringify(studies));
...
export function getStudyTemplateContent({ name }: any) {
